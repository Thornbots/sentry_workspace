...
- Launch `sim` with its GUI, not headless; the user watches the gz sim window
...
- `sentry_pkg`'s SLAM (`auto.launch.py`) depends only on `/pose` (via
  `pose_translator`) + `/scan`, no direct real-hardware dependency, so it works
...
- **rviz/TF now confirmed working end-to-end** (2026-07-20): scan renders in
  ...
  IP) breaking local discovery, now removed. Committed as `4598976 "fixed
...
- **gz-sim defaulting to the Intel iGPU instead of NVIDIA, confirmed fixed
  ...
  `__EGL_VENDOR_LIBRARY_FILENAMES` (first attempt) was **not** enough, since gz
  sim's on-screen GUI window uses GLX, not EGL, so it kept landing on Intel
  (verified via `nvidia-smi` showing 0% util / no processes even with the
  EGL var set). Fix: also set the same vars the host's `prime-run`
  ...
  needs). Verified via `nvidia-smi`: `ign gazebo server`/`gui` now show as real GPU
  processes using several hundred MB and nonzero utilization,
...
- `ros-humble-ros-gz` (provides `ros_gz_sim`/`ros_gz_bridge`/`ros_gz_image`,
  needed by `sim`) went missing after a container recreation, the same
...
- **Found (2026-07-24): the robot drives straight through obstacles in sim,
  ...
  demonstrate.
- **Add an EKF fusing `/odom` + `/scan` for `odom->root`**, replacing
  ...
  - Step 1: **done (2026-07-20)**. `ekf_node` now runs cleanly. Turned out
    ...
    container-local (like the GPU/udev fixes elsewhere in this doc), so it will
  ...
  - **Important correction (2026-07-20, discovered while implementing step
    ...
    - **(b), rejected.** Only trust corrections when the head happens to
      be at whatever angle it started at (no explicit "home" homing). Not
      viable per the user: the head changes angle too often for this
    ...
    - **(c), not implemented, noted for later.** Fork `rf2o` to re-query
      ...
      That's the actually-correct fix, letting `rf2o` work at any head angle,
      not just home. Bigger effort (maintaining a patched fork rather than
...
- **Added AMCL as a second, opt-in map->odom localization backend
  ...
  **Verified end-to-end in sim (2026-07-20)**: `ros2 launch sentry_pkg
  ...
  driving forward), confirming AMCL is actively scan-matching against the
  ...
  4.0.7). A recurring container-local footgun, not a one-off. Check
  `diagnostic_updater`'s installed version first if *any*
  nav2/robot_localization node dies with a "cannot open shared object file"
  error after a container recreation.
...
- **Consolidated `auto.launch.py`'s 3 separate localization args into one
  ...
  lookup for each, `map->odom` for slam/mapping/amcl and `odom->root` for
...
- **RESOLVED (2026-07-25): `ekf` mode now beats raw wheel odometry by 89%.**
  This supersedes the 2026-07-24 entry that used to live here (which
  ...
  - **Root cause: `sim`'s gpu_lidar published `angle_min=0.0,
    ...
    publish, so sim and hardware had silently disagreed on the scan
  ...
  - **`ekf.yaml` fusion assignment was also wrong** (`sentry_localization`
    ...
    velocity into the world using the filter's yaw, and a wrong yaw
...
- `localization_mode`: `slam` (default) / `mapping` / `amcl` / `none`, the
...
- `sim/sim/target_driver.py` publishes `nav_msgs/Odometry` on
  ...
  1.5184 rad FOV so dwell time stays above the ~10-sample convergence floor
  `point_to_cv_target`'s velocity filter needed at the time (that filter is
  gone as of 2026-07-28; the 47-sample measurement still holds, see
  `sim/README.md`'s path-geometry notes), even at high sweep speeds
  (measured: min 47 consecutive in-frustum samples at 8 m/s, the top of the
  default sweep). Advances state from
...
and `/target/ground_truth_odom` arrows. Fixed Frame is `odom`, with no map
